\doxysection{Bilinear Interpolation}
...
\begin{DoxyPre}
  ...
  \{
    ...
    uint16\_t numCols;
    float32\_t *pData;
\} \doxyref{arm\_bilinear\_interp\_instance\_f32}{p.}{structarm__bilinear__interp__instance__f32};
\end{DoxyPre}

\end{DoxyParagraph}
\begin{DoxyParagraph}{}
where {\ttfamily num\+Rows} specifies the number of rows in the table; {\ttfamily num\+Cols} specifies the number of columns in the table; and {\ttfamily p\+Data} points to an array of size {\ttfamily num\+Rows$\ast$num\+Cols} values. The data table {\ttfamily p\+Table} is organized in row order and the supplied data values fall on integer indexes. That is, table element (x,y) is located at {\ttfamily p\+Table[x + y$\ast$num\+Cols]} where x and y are integers.
\end{DoxyParagraph}
\begin{DoxyParagraph}{}
